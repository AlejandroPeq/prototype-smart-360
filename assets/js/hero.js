...
$(document).ready(function(){
	...
	function youtubeBackground() {
		...
		if (option_hero_background_youtube_controls === "on") {
			$(".hero .front-content").append(
				'<div class="controls">'
					+ '<i class="volume-button fa-solid fa-volume-high"></i>'
				+ '</div>'
			);
		}

		// Convert options
		let isMuted = option_hero_background_youtube_mute === "on";
		let loopVideo = option_hero_background_youtube_loop !== "off";
		let startAt = Number(option_hero_background_youtube_startPoint || 0);
		let endAt = Number(option_hero_background_youtube_endPoint || 0);
		let videoId = option_hero_background_youtube_url.replace(/.*v=/, "").trim();

		if (isMuted) {
			$(".volume-button").removeClass("fa-volume-high").addClass("fa-volume-xmark");
		}

		// Add player container
		$(".hero .bg-video").append('<div id="bg-youtube"></div>');
		...
		window.onYouTubeIframeAPIReady = function () {
			player = new YT.Player("bg-youtube", {
				...
				playerVars: {
					...
					playsinline: 1,
					start: startAt,
					end: endAt > startAt ? endAt : undefined,
					mute: isMuted ? 1 : 0,
				},
				events: {
					onReady: function (e) {
					...
					onStateChange: function (e) {
						// Loop manually when using custom end time
						if (loopVideo && endAt > 0 && e.data === YT.PlayerState.ENDED) {
		...
		$(".volume-button").click(function () {
			if (player.isMuted()) {
				player.unMute();
				$(".volume-button").removeClass("fa-volume-xmark").addClass("fa-volume-high");
			} else {
				player.mute();
				$(".volume-button").removeClass("fa-volume-high").addClass("fa-volume-xmark");
			}
		...
		function scaleYTBackground() {
			const iframe = document.querySelector(".hero .bg-video iframe");
			if (!iframe) return;
			...
			if (screenRatio > videoRatio) {
				// Screen is very wide → expand height
				iframe.style.width = w * 1.2 + "px";
				iframe.style.height = (w * 1.2) / videoRatio + "px";
			} else {
				// Screen is tall → expand width
				iframe.style.height = h * 1.2 + "px";
				iframe.style.width = h * 1.2 * videoRatio + "px";
			}
		}

		window.addEventListener("resize", scaleYTBackground);
		window.addEventListener("load", scaleYTBackground);
	}
